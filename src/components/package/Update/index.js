...
import {postSubscriptionPackageReset, updateSubscriptionPackageApiReset} from "../../../reducers";
...
const UpdatePackage = (props) => {
    ...
    const [handleUpdate,initialPackage] = useOutletContext();
    ...
    const {data, loading, error} = useSelector((state) => state.updateSubscriptionPackageApiSliceReducer);

    useEffect(() => {
        return function cleanup() {
            dispatch(updateSubscriptionPackageApiReset());
        };
    ...
    useEffect(() => {
        if(error){
            setConfirmation({
                show: true,
                title: "Error",
                text: "Error occurs while updating package"
                ,
                data: {},
                isUpdate: false,
                buttonYes:
    ...
    const handleAddPackage = (e) => {
        ...
        if (validate.isValid) {
            ...
            dispatch(updateSubscriptionPackageApi(data));
        } else {
                ...
                <Grid item xs={12} md={5.8} container justifyContent={"center"} sx={{height:{xs:"auto",md:"100%"}}} alignItems={"center"}>
                    <Grid item xs={10} container alignItems={"center"}>
                        <Grid item xs={12} container justifyContent={"center"}>
                            <CustomLabelHeaderLarge text={"Update"} color={"black"} fontWeight={"bold"}/>
                        </Grid>
